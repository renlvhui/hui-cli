...
export const templates: Map<string, TemplateInfo> = new Map([
    [
        'Vite-Vue3-Typescript-tempalte',
        {
            ...
            downloadUrl: 'git@github.com:renlvhui/admin.git',
...
export const checkVersion = async (name: string, version: string) => {
    ...
    if (need) {
        console.warn(
            `检查到yuhui最新版本： ${chalk.blackBright(latestVersion)}，当前版本是：${chalk.blackBright(version)}`
        );
        console.log(
            `可使用： ${chalk.yellow('npm install yuhui-cli@latest')}，或者使用：${chalk.yellow('yuhui update')}更新`
        );
    }
    return need;
};
